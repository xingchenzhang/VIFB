% This is the code for NSCT_SR image fusion algorithm.
...
function img = run_NSCT_SR(imgVI, imgIR, visualization)
    ...
    if size(img2,3)==1   
        imgf = nsct_sr_fuse(img1,img2,[2],D,overlap,epsilon);        
    elseif size(img1,3) == 1
        imgf = zeros(size(img2)); 
        for i=1:3
            imgf(:,:,i) = nsct_sr_fuse(img1,img2(:,:,i),[2],D,overlap,epsilon);         
        end   
    else
        imgf = zeros(size(img1)); 
        for i=1:3
            imgf(:,:,i) = nsct_sr_fuse(img1(:,:,i),img2(:,:,i),[2],D,overlap,epsilon);         
        end   
    toc;
    end
