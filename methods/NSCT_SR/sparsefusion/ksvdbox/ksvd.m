...
%两种模式：X是训练信号集，GAMMA是系数（为之），D是待训练的字典
...
%
%  [D,GAMMA,ERR] = KSVD(PARAMS) also returns the target function
%  values返回目标函数在迭代后的误差值
...
%
%
%  Required fields in PARAMS:必须有的参数
%  --------------------------
%
%    'data' - Training data.
...
% 例子：
% params.data = X;
% params.Tdata = k;   这个值主要用在KSVD的主循环中Sparse coding那一步，omp算法用到
% params.dictsize = m;
% params.iternum = 30;
% params.memusage = 'high';
%
%  Optional fields in PARAMS:可选参数
%  --------------------------
%
%    'testdata' - Validation data.测试数据
...
%
%    'iternum' - Number of training iterations.主循环迭代次数
%      Specifies the number of K-SVD iterations to perform. If not
%      specified, the default is 10.
%
%    'memusage' - Memory usage.决定omp算法的模式
...
perm = randperm(length(unused_sigs));
perm = perm(1:min(maxsignals,end));
err = sum((X(:,unused_sigs(perm)) - D*Gamma(:,unused_sigs(perm))).^2);
...
for j = 1:dictsize
  ...
  % replace atom
  if ( (max(Gj.^2) > muthresh^2) || (~replaced_atoms(j) && usecount(j)<use_thresh) )
    [y,i] = max(err);
    d = X(:,unused_sigs(perm(i)));
    D(:,j) = d/norm(d);
    unused_sigs = unused_sigs([1:perm(i)-1,perm(i)+1:end]);
