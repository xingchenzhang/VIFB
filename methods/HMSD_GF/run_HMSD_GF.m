% This is the code for HMSD_GF algorithm:
% Z. Zhou, M. Dong, X. Xie, and Z. Gao, ¡°Fusion of infrared
% and visible images for night-vision context enhancement,¡±
% Applied optics, vol. 55, no. 23, pp. 6480¨C6490, 2016
%
% The code of HMSD_GF is provided the authors of HMSD_GF.
% The interface is created by the authors of VIFB.

function img=run_HMSD_GF(imgVI, imgIR, visualization)

    %    The following is an implementation of "the HMSD fusion based on the guided filter (HMSD-GF)"
    ...
    tic;   
    if size(img2, 3) == 1
        fuseimage = HMSD_GF(img2, img1);
    elseif size(img1,3) == 1
        fuseimage = zeros(size(img2));
        for i=1:3
            fuseimage(:,:,i) = HMSD_GF(img2(:,:,i),img1);    
        end       
    else
        fuseimage = zeros(size(img2));
        for i=1:3
           fuseimage(:,:,i) = HMSD_GF(img2(:,:,i),img1(:,:,i));    
        end    
    end       
    toc;
    if visualization == 1
...
function res = HMSD_GF(img1, img2)
