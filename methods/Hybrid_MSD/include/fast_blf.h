...
*/
		...
		Data_array* const  result)
  {
    ...
    const base_type base_delta = base_max - base_min;////base=input
    
    const size_type small_width  = static_cast<size_type>((width  - 1) / space_sigma) + 1 + 2 * padding_xy; //////space_sigma=space_sampling
