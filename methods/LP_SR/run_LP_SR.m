% This is the code for MST_SR image fusion algorithm.
...
function img = run_LP_SR(imgA, imgB, visualization)
    ...
    % IR image
    image_input1 = imread(imgB.img);

    % VI image
    image_input2 = imread(imgA.img);
    ...
    elseif size(img1,3) == 1
        ...
        for i=1:3
            imgf(:,:,i) = lp_sr_fuse(img1,img2(:,:,i),level,3,3,D,overlap,epsilon);  
        end                
    else    
        imgf = zeros(size(img1)); 
        for i=1:3
            imgf(:,:,i) = lp_sr_fuse(img1(:,:,i),img2(:,:,i),level,3,3,D,overlap,epsilon);  
        end        
    end
